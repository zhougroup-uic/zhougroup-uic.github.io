...
function make_particle(pos, npatch::Int, patch_cov::Float64)
    bp     = body_patches(npatch)
    ctmax  = npatch > 0 ? 1.0 - 2.0*patch_cov/npatch : -2.0
    orient = random_orient()
    Particle(NTuple{3,Float64}(pos), orient, orient_patches(orient, bp), bp, npatch, ctmax)
end
...
function make_box(N::Int, L::Float64, rc::Float64,
                  npatch::Int, patch_cov::Float64, patch_e::Float64)
    parts = Particle[]
    n = ceil(Int, cbrt(N))
    for i in 0:n-1, j in 0:n-1, k in 0:n-1
        length(parts) == N && break
        pos = ((i+0.5)/n - 0.5, (j+0.5)/n - 0.5, (k+0.5)/n - 0.5)
        push!(parts, make_particle(pos, npatch, patch_cov))
    end
...
function mcmove!(b::Box, beta::Float64)
    ...
    if rand(RNG) < 0.5   # translation
        ...
        np = Particle((p.pos[1]+Δ[1], p.pos[2]+Δ[2], p.pos[3]+Δ[3]),
                      p.orient, p.patches, p.body, p.npatch, p.cos_max)
        ...
        np = Particle(p.pos, new_orient, p.patches, p.body, p.npatch, p.cos_max)
        set_patches!(np)
        en = particle_energy(b, np, i)
        if rand(RNG) < exp(-beta*(en-eo))
...
function mcswap!(e1::Box, e2::Box, mu::Vector{Float64}, beta::Float64)
    ...
    ghost = make_particle(Tuple(rand(RNG, 3).-0.5), dst.npatch, dst.patch_cov)
...
function main(infile="read.in", outfile="data.out", posfile="p.dat";
    ...
    println("beta=", beta, "  npatch=", npatch,
            "  patch_cov=", patch_cov, "  patch_e=", patch_e)

    e1 = make_box(N1, L1, rc, npatch, patch_cov, patch_e)
    e2 = make_box(N2, L2, rc, npatch, patch_cov, patch_e)
